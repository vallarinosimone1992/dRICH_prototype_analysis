...
void fitSigma(TH1D *h, bool out, bool Simulation){
  TF1 *f = new TF1("f","sqrt([0]*[0]/x + [1]*[1])",0,200);
  f->SetParameters(1.1,0.1);
  int min=0, max=0;
  if(out == false){
    if(Simulation==true && APPLY_QUANTUM_EFFICIENCY==false){
      min=60;//6;
      max=140;//22;
    }else{
      min=2;//6; //30 and 40 for simulation with QE but without lucite
      max=35;//28;
    }
  }else{
    if(Simulation==true && APPLY_QUANTUM_EFFICIENCY==false){
