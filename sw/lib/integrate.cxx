...
void TTreeIntegration(THeader *run){
  ...
  float gx0=0, gy0=0, gx1=0, gy1=0, gxa=0, gya=0, gxtheta=0, gytheta=0, tmpxtheta=0, tmpytheta=0;;
  ...
  TH2D *hD = new TH2D("hD","hD",300,-.01,.01,300,-.01,.01);	
  TH1D *hdX = new TH1D("hdX","hdX",100,-.01,.01);	
  TH1D *hdY = new TH1D("hdY","hdY",100,-.01,.01);	
  ...
  for(int i = 0; i < t->GetEntries(); i++){
    ...
    GEM_relative(&gx0,&gy0,&gx1,&gy1);
    AERO_computing(run,&gxa,&gya,&tmpxtheta,&tmpytheta,gx0,gy0,gx1,gy1);
    hD->Fill(tmpxtheta,tmpytheta);
    hdX->Fill(tmpxtheta);
    hdY->Fill(tmpytheta);
    tout->Fill();
    cout <<Form("GEM %lf %lf - %lf %lf - %lf %lf\n",gx0,gy0, gx1, gy1,tmpxtheta,tmpytheta);
  }
  printEnd();
  run->UpGEMxRunOff=0;
  run->UpGEMyRunOff=0;
  ...
  TF2 *fTheta = new TF2("fTheta","bigaus(0)",-.1,.1,-.1,.1);
  fTheta->SetParameters(10,-0.04,.01,-0.04,.01);
  hD->Fit(fTheta,"Q","",-.01,0);
  ...
  hD->Draw("colz");
  c->cd(2);
  TF1 *fX = new TF1("fX","gaus",-1,1);
  hdX->Draw();
  hdX->Fit(fX,"Q","",hdX->GetBinCenter(hdX->GetMaximumBin())-0.003,hdX->GetBinCenter(hdX->GetMaximumBin())+0.003);
  c->cd(3);
  TF1 *fY = new TF1("fY","gaus",-1,1);
  hdY->Draw();
  hdY->Fit(fY,"Q","",hdY->GetBinCenter(hdY->GetMaximumBin())-0.003,hdY->GetBinCenter(hdY->GetMaximumBin())+0.003);
  ...
  for(int i = 0; i < tout->GetEntries(); i++){
    ...
    AERO_computing(run,&gxa,&gya,&gxtheta,&gytheta,tmpx0,tmpy0,tmpx1,tmpy1);
    gxtheta-=fX->GetParameter(1);
    gytheta-=fY->GetParameter(1);
    tGXtheta->Fill();
    tGYtheta->Fill();
    cout <<Form("GEM second %lf %lf - %lf %lf - %lf %lf\n",gx0,gy0, gx1, gy1,gxtheta,gytheta);
  }
  printEnd();
